...
title, legend position, background, and title, gridlines etc. `ggplot` provides 
complete themes that users can use, such as `theme_bw()` and `theme_minimal()`.

As stated in previous section, customizing themes is imperative in data 
...
visualizations that provide information can be mistakenly removed.

Give plot examples.

Theme is especially important in a research book where there are many authors
contribute to the book. Without a set theme, each authors will have their own
visualization styles for their research. Themes will allow research books to 
