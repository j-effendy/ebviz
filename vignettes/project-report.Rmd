...
thicker. Vertical Major grid lines and minor grid lines on both axis are removed. 
Horizontal major grid lines are not removed as they can aid readers to 
approximate and compare values between data points. The grid lines, as shown in 
figure \@ref(fig:seagull-scatter), are visually muted so that it does not 
distract the data points. Cleveland [-@cleveland1993visualizing] found that 
grid lines enhance readers' perceptions in making visual comparison and accuracy 
...
```{r theme-comparison, fig.cap="Comparison between theme_wombat (left) and theme_bw (right).", fig.height=14, fig.width=11}
...
the difference between `dataviz` potential themes and `theme_bw()` is the size
...
underlying data. Another way of maximising data-ink ratio is by minimising
the margin around the plot. As shown in figure \@ref(fig:theme-comparison) 
there are no extra space between the bars and lines with the axis lines in the 
plots with `theme_seagull()` and `theme_wombat()`.
