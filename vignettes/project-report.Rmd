...
an edited book that have a cohesive design. This project aims to reduce 
...
colour scale functions that work with `ggplot2`. This will allow users
...
`%+replace%` operator is also used to develop `theme_wombat()`. It can be seen
...
non-data components while still preserving light coloured grid lines.
...
[@treisman1985preattentive]. The use of colour in data visualisation can facilitate
this "pop-out" as it makes some elements to be easier to be found.

```{r pop-out, fig.cap="This plot shows the same data with groups highlighted with either colour (left hand panel) or shape (right hand panel). From this we see that it is easier to find the odd ones by using colour"}
set.seed(20211028)
...
`pals` packages are going to be used to build the colour palettes 
and assess them. The chosen colour palettes need to be assessed to ensure
that they achieve their intended purpose. For example, qualitative colour palette
needs to avoid implicit ordering as this is intended for showing coding categorical 
information. In addition, it needs to be ensured that people with colour vision
impairment are able to distinguish the different colours in the chosen palettes.
`pal.safe()` function from `pals` package allows us to see how colour palettes
are seen by deutan, protan, and tritan colour-blind.
...
```{r color-blind, fig.height=8, fig.cap="colour palettes as seen by people with vision impairment"}
...
```{r harmonic, fig.cap="HCL colour spectrum plot for Harmonic palette"}
...
```{r qualitative, fig.height=3, fig.cap="HCL colour spectrum plot for Quokka and Quoll palette"}
par(mfrow=c(1,2))
...
tritan colour-blindness (see figure \@ref(fig:color-blind)). 
...
```{r sequential, fig.height=3, fig.cap="HCL colour spectrum plot for Seal and Snake palette"}
par(mfrow=c(1,2))
...
Moreover, multi-hue sequential palettes avoids the use of extreme colours, as well
as allow viewers to better discriminate the middle colours [@zeileis2019colorspace]. 
...
This means that colour palettes will allow readers with colour vision deficiency
...
palette based on these values, and it will automatically choose the neutral colour.

```{r diverging, fig.height=3, fig.cap="HCL colour spectrum plot for Dingo and Dolphin palette"}
par(mfrow=c(1,2))
specplot(dingo_palette, main = "Dingo Palette")
specplot(dolphin_palette, main = "Dolphin Palette")
```

One condition needs to be met when choosing a colour pair for a diverging
palette: the colour pair should not create colour confusions: naming confusions, 
colour vision impairment, and simultaneous contrast confusion 
[@brewer1996guidelines]. Naming confusion happens when people use two 
basic colour names for the same colour. On the other hand, simultaneous contrast
refers to a phenomena where colours are affected by their opposite. For instance,
...
prevented by avoiding selecting opposite colour pairs for diverging palettes
...
to create the chosen `ggplot2` theme and colour schemes discussed in the previous
...
the package. Another benefit of using an R package is that updates to the theme
and colour schemes are not time consuming. Any updates to themes and colour schemes
...
One of the most important components of a package is the R codes. One firm
rule for an R package is that its R functions need to be stored in R scripts 
[@wickham2015r]. Following this rule, The `dataviz` put its R codes in  R 
scripts, which are stored in `R` directory. The chosen colour palettes can be 
accessed from `scale_colour_dataviz` and `scale_fill_dataviz` functions which
...
package [@wickham2015r]. In the absence of documentations, users wont a guide
that assists them to use the package properly. Proper documentations also assist
the authors of the package for future time. This assists authors to remember the
purpose of the functions. One type of documentation is an object documentation;
this can be accessed by `?` or `help()`. Information such as function's 
description, usage, argument, and examples can be found in the documentation.
`roxygen2`, `devtools`, and `usethis` packages are used in creating object 
documentation.

Another crucial aspect to package development is testing. While testing add
extra works to package development, it ensures that the package works
as intended. Package `testthat` and `usethis` are used to write the tests for
`dataviz` package. A test that was used for this package is to ensure that 
users use valid input when using `scale_colour_dataviz` and `scale_fill_dataviz`.
...
One of the challenges in producing a research book is create a book that has
a consistent look/design. Typically, there is a group of authors that work on
a chapter of an edited book. These authors will have their own way/style in
producing their visualizations for the book. Not only do these visualizations
need to have a consistent design, but they also need to be readable and 
accessible.
...
section in an efficient manner. To achieve this objective, a ggplot theme was 
developed for the project. In addition, several colour scales for various
data types are also created. The chosen theme was developed based on Tufte's
principles of graphical excellence. This is to ensure the viewers to extract
information as much as possible from the visualizations. Moreover, the colour
palettes were developed with a colour space that was based on human perception.
Furthermore, how colours seen by people with visual impairment was taken into 
consideration to ensure that the colour palettes are accessible. The theme and
colour schemes are bundled in an R package for efficiency.
...
this palette is specifically used for show **un-ordered** categorical information.
...
accessibility proves to be difficult. Figure \@ref(fig:color-blind) shows that
...
four colours while still ensuring that people with color deficiency are able to 
distinguish the colours from one another.

Other potential limitations: lack of testings, lack of colour assessments.
