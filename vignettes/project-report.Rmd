...
2. Chroma: colourfulness (colour intensity)
...
`quokka_palette()` and `quoll_palette()`. Figure \@ref(fig:qualitative) shows 
...
tritan colour-blindness (see figure \@ref(fig:color-blind)). However, removing
...
`snake_palette()`. As shown in figure \@ref(fig:sequential),
...
scripts, which are stored in `R` directory. The R codes required to construct 
the chosen colour palettes and `ggplot2` themes discussed in previous sections are 
stored in `colour.R` and `theme.R` respectively.
...
this can be accessed by `?` or `help()`. `roxygen2` and `devtools` packages are
used in creating object documentation. 
