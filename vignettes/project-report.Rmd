...
## The Role of Colors in Data Visualisation (300)
...
In addition to assisting viewers to differentiate one element to another, 
...
human visual perceptions [@smith1978color]. While HSV is easy to use, it has 
several drawbacks: it is fairly difficult set HSV coordinates for colors that
are in harmony [@zeileis2006choosing]. Furthermore, HSV encourages the use of 
colors that are highly saturated. While these colors are good to attract viewers' 
attention, it is difficult to look at for a long time [@zeileis2009escaping].

Another color model that was developed based on human perception is 
