---
title: "mrpviz: An R package to create accessible and cohesive R plots for an edited research book"
...
and colour palettes are bundled together in an R package called `mrpviz`.
...
and colour scales will be wrapped in an R package called `mrpviz`. The R package
...
report contributes themes, colour schemes, and a package called `mrpviz`.
...
are moiré effect (the illusion of vibration caused by excessive patterns), 
heavy grid lines, noisy background, and a fake perspective (i.e. 3-D charts).
...
readers and sometimes result in graphical clutters. However, maximizing the 
...
organised fashion. One option is to make the colour of non-data components (axis, labels, 
grid lines, borders) to be grey, and the data component to be brightly
coloured, making the data to be the focus of attention [@stone2006choosing]. 
This was found to assist readers to better understand the roles and relationship between 
...
The default `ggplot2` theme is `theme_grey()`, with grey background and white 
...
on top of a white plot background increases non-data components.
...
`mrpviz`: `theme_seagull()` and `theme_wombat()`.
...
the differences between `mrpviz` potential themes and `theme_bw()` is the size
...
With a simple visual feature, some objects can be easily recognised without 
requiring much attention. The target object will "pop-out" 
in field of vision regardless of how many distractors surrounding it 
...
Figure \@ref(fig:pop-out) is an example, which is adapted from Healy's [-@healy2018data] 
book about data visualization, that demonstrates how easy it is for colour to be 
recognised in a visualization. The figure contains three "odd" observations that 
are different than the rest. To represent this, the first panel uses colour to differentiate 
...
```{r color-blind, fig.height=8, fig.cap="Colour palettes as seen by people with colour vision impairment. The first six colour palettes the palettes that were developed for this project. The last palette, Harmonic palette, is a colour palette from `pals` package.", fig.subcap=c("","","","","","",""), fig.ncol = 3, fig.align='center', out.width="75%", out.height="25%"}
...
Panel (g) in figure \@ref(fig:specplot) shows an example of a qualitative colour palette from 
...
```{r specplot, fig.height=3, fig.cap="HCL colour spectrum plot for colour palettes of choice. The first six colour palettes the palettes that were developed for this project. The last palette, Harmonic palette, is a colour palette from `pals` package.", fig.subcap=c("","","","","","",""), fig.ncol = 2,fig.align='center', out.width="50%", out.height="25%"}
...
Quokka and Quoll palettes. Panel (a) and (b) in figure \@ref(fig:specplot) shows 
...
Snake palettes. As shown in panel (c) and (d) in figure \@ref(fig:specplot),
...
Three conditions need to be met by a diverging palette to avoid colour 
confusions. Colour confusions are caused by naming confusions, colour vision 
...
colour (e.g. a colour was called red when presented once, then pink at 
...
These palettes are shown in panel (e) and (f) in figure \@ref(fig:specplot).
...
(shown in figure \@ref(fig:color-blind)). Another reason for choosing these pairs
...
package called `mrpviz`. R packages are extensions to R that can comprise of 
...
repository. For this project, the `mrpviz` package will be stored in GitHub.
...
Comprehensive R Archive Network (CRAN). CRAN, however, is not suitable for this 
package as `mrpviz` has a fairly small scope and is intended for a small group 
...
In the next section, the report will discuss how the `mrpviz` package was 
...
[@wickham2015r]. Following this rule, the `mrpviz` has its R code in  R 
scripts, which are stored in `R` directory. The chosen colour palettes can be 
accessed from the `scale_colour_mrpviz` and `scale_fill_mrpviz` functions which
...
The `roxygen2` [@roxygen2], `devtools` [@devtools], and `usethis` [@usethis] 
...
messages to appear when required (and not appear when they are not supposed to).
...
the tests for the `mrpviz` package. One example of a test that was used for 
this package is one that ensures that users use valid input when using 
`scale_colour_mrpviz` and `scale_fill_mrpviz`.
...
avoiding implied ordering for people with colour visual impairment, there are still 
...
four colours while still ensuring that people with colour impairment are able to 
distinguish the colours from one another.

One area of future work for this project is to work on writing additional 
testings. For instance, a test can be developed to ensure that a warning message will appear
